...
let lesson = Array.from(document.getElementsByClassName("lesson"));
let lessonContent = document.getElementsByClassName("information");
...
for (let i = 0; i < lesson.length; i++) {
    lesson[i].addEventListener("click", () => {
        for (let j = 0; j < lesson.length; j++) {
            lesson[j].classList.remove("active");
            lessonContent[j].classList.add("hide-content");
        }

        lesson[i].classList.add("active");
        lessonContent[i].classList.remove("hide-content");
    });
}